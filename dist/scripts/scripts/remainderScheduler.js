 ...
 */
async function getSheetService(serviceAccountFile) {
    try {
 ...
 */
async function readFromSheet(service, config) {
    try {
 ...
 */
function cleanRecipientName(recipientName) {
    return recipientName.replace(/[@-]/g, '').trim();
}
/**
 * Check if cleaned recipient name exists in campaign receipt names
 */
function isRecipientInCampaignReceiptNames(cleanedName, campaign) {
    if (!campaign.recieptNames || !Array.isArray(campaign.recieptNames) || !cleanedName)
        return false;
    ...
    return campaign.recieptNames.some((receiptName) => {
        if (!receiptName)
            return false;
        ...
        if (cleanedNameParts.length >= 2 && receiptNameParts.length >= 2) {
            const matchingParts = cleanedNameParts.filter(part => part.length > 2 && receiptNameParts.some(receiptPart => receiptPart.length > 2 && (receiptPart.includes(part) || part.includes(receiptPart))));
        ...
        if (cleanedNameParts.length === 1 && cleanedNameParts[0] && cleanedNameParts[0].length > 2) {
            return receiptNameParts.some(receiptPart => receiptPart.length > 2 && cleanedNameParts[0] && (receiptPart.includes(cleanedNameParts[0]) || cleanedNameParts[0].includes(receiptPart)));
        }
        return false;
    });
}
 ...
 */
async function getUnpaidMembers(campaign, sheetEntries, startDate, endDate) {
    const unpaidMembers = [];
    ...
    for (const campaignMember of campaign.members) {
        ...
        const memberPaymentEntries = campaignRelevantEntries.filter(entry => entry.memberId === member.memoId);
 ...
 */
function getUnpaidReceiptIds(member, campaign, campaignRelevantEntries) {
    ...
    memberPaymentEntries.forEach(entry => {
        ...
        if (campaign.receiptNames && Array.isArray(campaign.receiptNames)) {
            campaign.receiptNames.forEach((receiptName, index) => {
    ...
    return campaign.receiptIds.filter((receiptId) => !paidReceiptIds.has(receiptId));
}
/**
 * Check if a reminder has already been sent for a member in a campaign
 */
async function hasReminderBeenSent(memberId, campaignId, reminderType) {
 ...
 */
async function recordReminderSent(memberId, campaignId, reminderType) {
    try {
 ...
 */
async function getMembersNeedingFollowUpReminders() {
    ...
    for (const reminder of firstReminders) {
        const hasFinalReminder = await hasReminderBeenSent(reminder.memberId, reminder.campaignId, ReminderType.FINAL_REMINDER);
        if (!hasFinalReminder) {
 ...
 */
async function sendReminder(member, campaign, whatsappSession, reminderType = ReminderType.FIRST_REMINDER, unpaidReceiptIds = []) {
    try {
 ...
 */
async function checkIfMemberHasPaid(member, campaign, allEntries) {
